...
class ResUsers(models.Model):
    ...
    def password_match_message(self):
        ...
        if pwd_params["lower"]:
            message.append(
                self.env._(
                    "\n* Lowercase letter (at least %(lower)s characters)",
                    lower=pwd_params["lower"],
                )
            )
        if pwd_params["upper"]:
            message.append(
                self.env._(
                    "\n* Uppercase letter (at least %(upper)s characters)",
                    upper=pwd_params["upper"],
                )
            )
        ...
        if pwd_params["minlength"]:
            message = [
                self.env._(
                    "Password must be %(minlength)s characters or more.",
                    minlength=pwd_params["minlength"],
                )
            ] + message
    ...
    def _validate_pass_reset(self):
        ...
        for user in self:
            ...
            if write_date + delta > datetime.now():
                raise UserError(
                    self.env._(
                        "Passwords can only be reset every %(minimum_hours)s hour(s). "
                        "Please contact an administrator for assistance.",
                        minimum_hours=pwd_params["minimum_hours"],
                    )
                )
        return True
    ...
    def _check_password_history(self, password):
        ...
        for user in self:
            ...
            if recent_passes.filtered(
                ...
                raise UserError(
                    self.env._(
                        "Cannot use the most recent %(history)s passwords",
                        history=pwd_params["history"],
                    )
                )
